class Cube{
    ...
    compute() {
        ...
        for (let i = 0; i < this.vertices.length; i+=3){
            vertex.elements = [this.vertices[i], this.vertices[i+1], this.vertices[i+2]];
            let uvIndex = (i/3) * 2;
            let u = this.uvs[uvIndex];
            let v = this.uvs[uvIndex+1];

            chunkVertexData.push(...this.matrix.multiplyVector3(vertex).elements, u, v, ...this.baseColor, this.texColorWeight);
        }
    }
}
