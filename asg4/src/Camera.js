...
class Camera {
  constructor() {
    this.fov = 60;
    this.eye = new Vector3([8, 4, 18]);
    this.at = new Vector3([16, 4, 22]);
    ...
    this.projMatrix = new Matrix4().setPerspective(
      this.fov,
      camCanvas.width / camCanvas.height,
      0.1,
      1000,
    );
  }
  ...
  panPitchDegrees(angle) {
    ...
    if (
      Vector3.dot(f, this.up) > Math.cos((clampAngle * Math.PI) / 180) &&
      angle > 0
    ) {
      return;
    }
    ...
    if (
      Vector3.dot(f, this.up) < -1 * Math.cos((clampAngle * Math.PI) / 180) &&
      angle < 0
    ) {
      return;
    }
